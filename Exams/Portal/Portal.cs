    ...
    public class Portal
    {
        ...
        private static void MoveVertical(int dir, List<List<char>> matrix, Position player)
        {
            ...
            else
            {
                ...
                {
                    nextRow = matrix
                        .LastOrDefault(row => row.Count > player.Col && matrix.IndexOf(row) < player.Row);

                    if (nextRow == null)
                    {
                        nextRow = matrix
                            .Last(row => row.Count > player.Col && matrix.IndexOf(row) >= player.Row);
                    }
                }
                else
                {
                    nextRow = matrix
                        .FirstOrDefault(row => row.Count > player.Col && matrix.IndexOf(row) > player.Row);

                    if (nextRow == null)
                    {
                        nextRow = matrix
                            .First(row => row.Count > player.Col && matrix.IndexOf(row) <= player.Row);
                    }
                }
